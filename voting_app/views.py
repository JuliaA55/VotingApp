...
def home(request):
    return render(request, 'votings/vote.html')
@csrf_exempt 
...
def category_book(request):
    newBooks = Book.objects.extra(where=["\"voting_app_book\".\"isNew\" = 'True'"])[:6]
    setBooks = Book.objects.extra(where=["\"voting_app_book\".\"isSet\" = 'True'"])[:6]
    recommendedBooks = Book.objects.extra(where=["\"voting_app_book\".\"isRecommended\" = 'True'"])[:6]
    categories = Category.objects.all()
...
def gift_list(request):
    gifts = Other.objects.extra(where=["\"voting_app_other\".\"is_gift\" = 'True'"])
    return render(request, 'books/gift_list.html', {'gifts': gifts})

def game_list(request):
    games = Other.objects.extra(where=["\"voting_app_other\".\"is_game\" = 'True'"])
    return render(request, 'books/game_list.html', {'games': games})

def notebook_list(request):
    notebooks = Other.objects.extra(where=["\"voting_app_other\".\"is_notebook\" = 'True'"])
